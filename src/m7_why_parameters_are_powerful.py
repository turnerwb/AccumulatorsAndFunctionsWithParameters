...
def better_draw_circles(n):
    ...
    -- Constructs and draws 21 rg.Circle objects such that:
         -- Each is centered at (200, 200)
         -- They have radii:  0  n  2n  3n  4n ... 20n, respectively.
         -- Pauses 0.05 seconds after rendering each.
    -- Waits for the user to press the mouse, then closes the window.
    """
...
def run_test_even_better_draw_circles():
    ...
    colour = 'green'
    number = 25
    even_better_draw_circles(delay, size, colour, number)


def even_better_draw_circles(delay, size, colour, number):
    window = rg.RoseWindow(400, 400)

    center = rg.Point(200, 200)
    for k in range(number):
        circle = rg.Circle(center, size * k)
        circle.outline_color = colour
        circle.attach_to(window)
        window.render(delay)  # Pauses for 0.05 seconds after rendering.
